...
PTAU_MAX=4096
CONSTRAINT_COUNT=$(npx snarkjs r1cs info build/entros_hamming.r1cs 2>&1 | grep -i "constraints" | grep -oE '[0-9]+' | head -1)
...
npx snarkjs zkey export verificationkey build/entros_hamming_final.zkey keys/verification_key.json
